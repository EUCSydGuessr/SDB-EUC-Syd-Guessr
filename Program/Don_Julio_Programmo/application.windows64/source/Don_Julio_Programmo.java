...
public class Don_Julio_Programmo extends PApplet {
...
public void setup() {
  ...
  Knap1Setting = cp5.addButton("Forste_sal").setLabel("Første Sal").setPosition(770, 50).setSize(100, 30).setFont(fontSmallButt).setColorBackground(0xff060a80).setColorActive(0xff252dfa).updateSize();

  Knap2Setting = cp5.addButton("Stueetage").setPosition(770, 90).setSize(100, 30).setFont(fontSmallButt).setColorBackground(0xff252dfa).setColorActive(0xff252dfa).updateSize();

  Knap3Setting = cp5.addButton("Kaelder").setLabel("Kælder").setPosition(770, 130).setSize(100, 30).setFont(fontSmallButt).setColorBackground(0xff060a80).setColorActive(0xff252dfa).updateSize();

  Knap4Setting = cp5.addButton("Guess").setPosition(100, 550).setSize(550, 100).setFont(fontBigButt).setColorBackground(0xff077eda);

  Knap5Setting = cp5.addButton("Next").setPosition(100, 550).setSize(550, 100).setFont(fontBigButt).setColorBackground(0xff077eda).setVisible(false);

  Knap6Setting = cp5.addButton("Restart").setPosition(100, 550).setSize(550, 100).setFont(fontBigButt).setColorBackground(0xff077eda).setVisible(false);
}
...
public void draw() {
  ...
  //Billede på skærm
  while (q <= 0 & pStop <= 4) {
    q = q + PApplet.parseInt(random(1, 26));
    if (q == p1) q = 0;
    if (q == p2) q = 0;
    if (q == p3) q = 0;
    if (q == p4) q = 0;
...
//Visuelt stuff, der ændrer sig efter man har gættet.
  if (guessMade == true) {
    ...
    if (faktiskEtage == 1)Knap3Setting.setColorBackground(0xff04d119);
    if (faktiskEtage == 2)Knap2Setting.setColorBackground(0xff04d119);
    if (faktiskEtage == 3)Knap1Setting.setColorBackground(0xff04d119);
  }
  stroke(0);
  fill(0);
  textSize(45);
...
public void Guess() {
  if (ringPlaced == true) {
    ...
    if (koordForskelScore<=5.5f) {
      subtraheringAfScore = 0;
    } else {
      subtraheringAfScore = PApplet.parseInt(koordForskelScore);
    }
...
public void Next() {
  ...
  Knap1Setting.setColorBackground(0xff060a80);
  Knap2Setting.setColorBackground(0xff060a80);
  Knap3Setting.setColorBackground(0xff060a80);
  if (etage == 1)Knap3Setting.setColorBackground(0xff252dfa);
  if (etage == 2)Knap2Setting.setColorBackground(0xff252dfa);
  if (etage == 3)Knap1Setting.setColorBackground(0xff252dfa);
...
public void Forste_sal() {
  ...
  Knap1Setting.setColorBackground(0xff252dfa);
  Knap2Setting.setColorBackground(0xff060a80);
  Knap3Setting.setColorBackground(0xff060a80);
}
...
public void Stueetage() {
  ...
  Knap2Setting.setColorBackground(0xff252dfa);
  Knap1Setting.setColorBackground(0xff060a80);
  Knap3Setting.setColorBackground(0xff060a80);
}
...
public void Kaelder() {
  ...
  Knap3Setting.setColorBackground(0xff252dfa);
  Knap1Setting.setColorBackground(0xff060a80);
  Knap2Setting.setColorBackground(0xff060a80);
}
...
public void mouseWheel(MouseEvent spin) {
  if (insideGuessField) {
    ...
    if (zoomAmount == 0.75f) borderMin = true;
    else if (zoomAmount == 4.0f) borderMax = true;
    else {
      borderMin = false;
      borderMax = false;
    }

    if (zoomAmount >= 0.75f && zoomAmount <= 4.0f) {
      if (!borderMin && !borderMax)zoomAmount = zoomAmount + e;
      else if (borderMin && e > 0.0f)zoomAmount = zoomAmount + e;
      else if (borderMax && e < 0.0f)zoomAmount = zoomAmount + e;
    }
...
class RandomBillede {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Don_Julio_Programmo" };
    if (passedArgs != null) {
